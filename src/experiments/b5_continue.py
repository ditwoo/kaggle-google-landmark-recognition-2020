...
def get_class_weights():
    ...
    counts = Counter(train["landmark_id"].values)
    weights = np.zeros(len(landmark_map))
    for lid, cnt in counts.most_common():
        weights[landmark_map[lid]] = cnt
...
def experiment(logdir: Path, device: torch.device) -> None:
    ...
    load_checkpoint("./logs/effnet-b5/stage_0/last.pth", model)
    ...
    with TensorboardLogger(tb_dir) as tb:
        ...
        train_loader, valid_loader = get_loaders(stage, train_bs=48, valid_bs=128)
...
def main() -> None:
    experiment_name = "effnet-b5-continue1"
